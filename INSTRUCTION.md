...
1. Открыть GIt Bash и перейти в папку, куда хотите клонировать репозиторий:
   ```
   cd /d/123/lolkek/
   ```
...
Для корректного отображанения и работы Markdown нужно, чтобы кодировка ваших .md-файлов была UTF-8 **БЕЗ** BOM. BOM в начале файла может ломает работу заголовочного тега `#`.
